...
namespace FlatBuffers
{
...
#if UNSAFE_BYTEBUFFER
		System.Action<IntPtr> _free = null;
		public ByteArrayAllocator(IntPtr p, int l, System.Action<IntPtr> free)
		{
			Length = l;
			_free = free;
			unsafe
			{
				Buffer = (byte*)p.ToPointer();
			}
		}
#endif
    ...
    public class ByteBuffer : IDisposable
    {
		...
		public ByteBuffer(IntPtr pointer, int length, System.Action<IntPtr> free) : this(pointer, length, 0, free) { }
        ...
        {
			byte[] ByteArray = _buffer.ByteArray;
			if(ByteArray != null)
			{
			...
			else
			{
				unsafe
				{
					int cacheSize = (len + 512) / 512 * 512;
					if (_caches == null || _caches.Length < cacheSize)
					{
						_caches = new byte[cacheSize];
					}
					Marshal.Copy(new IntPtr(_buffer.Buffer + startPos), _caches, 0, len);
					return Encoding.UTF8.GetString(_caches, 0, len);
				}
			}
#endif
			return "";
